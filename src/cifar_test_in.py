...
def run_full_code_in(device, model_file, test_data_file, output_file, n):
	...
	class ResNetBlock(nn.Module):
		def __init__(self, numlayer, n):
			...
			self.instance_norm1 = Instance_Normalisation(numlayer)
			self.conv2 = nn.Conv2d(numlayer, numlayer, 3, padding = 1)
			self.instance_norm2 = Instance_Normalisation(numlayer)
		def forward(self, x):
			y = x
			x = self.conv1(x)
	...
	class ResNet_Downsample(nn.Module):
		def __init__(self, numlayerin, numlayerout, n):
			...
			self.instance_norm1 = Instance_Normalisation(numlayerout)
			self.conv2 = nn.Conv2d(numlayerout, numlayerout, 3, padding = 1)
			self.instance_norm2 = Instance_Normalisation(numlayerout)
	...
	class ResNet(nn.Module):
		def __init__(self, n1, r1):
			...
			self.instance_norm1 = Instance_Normalisation(16)
